...
class Fourier(Basis):
    ...
    def _transform_to_grid_complex(self, data, axis, scale):
        coeff_expand = np.zeros(int(np.ceil(self.N*scale)), dtype=np.complex128)
        coeff_expand[:np.ceil(self.N/2).astype(int)] = data[:np.ceil(self.N/2).astype(int)]
        coeff_expand[-np.ceil(self.N/2).astype(int)+1:] = data[-np.ceil(self.N/2).astype(int)+1:]
        grid_data = scipy.fft.ifft(coeff_expand)
        return grid_data*self.N*scale

    def _transform_to_coeff_complex(self, data, axis):
        coeff_data = scipy.fft.fft(data)
        coeff_data[len(data)//2]=0
        coeff_shrink = np.zeros(self.N, dtype=np.complex128)
        coeff_shrink[:np.ceil(self.N/2).astype(int)] = coeff_data[:np.ceil(self.N/2).astype(int)]
        coeff_shrink[-np.ceil(self.N/2).astype(int)+1:] = coeff_data[-np.ceil(self.N/2).astype(int)+1:]
        return coeff_shrink/len(data)

    def _transform_to_grid_real(self, data, axis, scale):
        complex_data = np.zeros(int((self.N//2)*scale)+1, dtype=np.complex128)
        complex_data[:self.N//2].real = data[::2]
        complex_data[:self.N//2].imag = data[1::2]
        grid_data = scipy.fft.irfft(complex_data)
        return grid_data*self.N*scale/2
        

    def _transform_to_coeff_real(self, data, axis):
        coeffs = scipy.fft.rfft(data)
        coeff_data = np.zeros(2*(len(coeffs)-1))
        coeff_data[::2] = coeffs.real[:len(coeffs)-1]
        coeff_data[1::2] = coeffs.imag[:len(coeffs)-1]
        coeff_shrink = coeff_data[:self.N]
        return coeff_shrink/len(data)*2
